...
type ColumnType = ColumnDef<MaintenanceInterface, unknown>;
...
export function MaintenanceListPage(props: MaintenanceListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getMaintenances({
        ...
        order: params.order,
        searchTermKeys: ['description.contains', 'maintenance_status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MaintenanceInterface>>(
    () => `/maintenances?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: MaintenanceInterface) => {
    if (hasAccess('maintenance', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/maintenances/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.maintenance_date ? format(parseISO(record?.maintenance_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    { id: 'description', header: 'Description', accessorKey: 'description' },
    { id: 'cost', header: 'cost', accessorKey: 'cost' },
    { id: 'maintenance_status', header: 'Maintenance Status', accessorKey: 'maintenance_status' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/maintenances/view/${record.id}`} passHref legacyBehavior>
                <Button
